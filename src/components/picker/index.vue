<template>
  <div class="ui-picker">
    <ui-mask :show.sync="show" @click="clickMask"></ui-mask>
    <transition name="ui-slide">
      <div class="weui-picker" :class="{'ui-picker__toggle': show}">
        <div class="weui-picker__hd" v-show="header">
          <a class="weui-picker__action" @click="cancel($event)">{{t('cancel')}}</a>
          <a class="weui-picker__action" @click="confirm">{{t('confirm')}}</a>
        </div>
        <div class="weui-picker__bd">
          <div class="weui-picker__group"
            v-for="(one, index) in currentData"
            :key="index"
            :id="`ui-picker-${uuid}-${index}`">
          </div>
        </div>
      </div>
    </transition>
  </div>
</template>
<script>
import uiMask from '../mask';
import Scroller from './scroller';
import Manager from './chain';
import value2name from './value2name';
import locale from '../../mixins/locale';

export default {
  name: 'UiPicker',
  mixins: [locale],
  components: {
    uiMask,
  },
  created() {
    if (this.columns !== 0) {
      const length = this.columns;
      this.store = new Manager(this.data, length, this.fixedColumns);
      this.currentData = this.store.getColumns(this.value);
    }
  },
  mounted() {
    this.uuid = Math.random().toString(36).substring(3, 8);
    this.$nextTick(() => {
      this.render(this.currentData, this.currentValue);
    });
  },
  props: {
    show: false,
    header: false,
    data: Array,
    columns: {
      type: Number,
      default: 0,
    },
    fixedColumns: {
      type: Number,
      default: 0,
    },
    value: Array,
    itemClass: {
      type: String,
      default: 'scroller-item',
    },
    columnWidth: Array,
  },
  methods: {
    clickMask() {
      this.$emit('update:show', false);
    },
    getNameValues() {
      return value2name(this.currentValue, this.data);
    },
    getId(i) {
      return `#ui-picker-${this.uuid}-${i}`;
    },
    cancel($event) {
      this.$emit('cancel', $event);
    },
    confirm($event) {
      this.$emit('confirm', this.value, $event);
    },
    render(data, value) {
      this.count = this.currentData.length;
      const ui = this;
      if (!data || !data.length) {
        return;
      }
      const count = this.currentData.length;
      // set first item as value
      if (value.length < count) {
        for (let i = 0; i < count; i += 1) {
          if (process.env.NODE_ENV === 'development' &&
            typeof data[i][0] === 'undefined' &&
            Array.isArray(this.data) &&
            this.data[0] &&
            typeof this.data[0].value !== 'undefined' &&
            !this.columns) {
            console.error('[error] 渲染出错，如果为联动模式，需要指定 columns(列数)');
          }
          this.$set(ui.currentValue, i, data[i][0].value || data[i][0]);
        }
      }

      for (let i = 0; i < data.length; i += 1) {
        /**
         * Still don't know why this happens
         */
        if (!document.querySelector(ui.getId(i))) {
          return;
        }

        ui.scroller[i] && ui.scroller[i].destroy();
        ui.scroller[i] = new Scroller(ui.getId(i), {
          data: data[i],
          defaultValue: value[i] || data[i][0].value,
          itemClass: ui.item_class,
          onSelect(v) {
            ui.$set(ui.currentValue, i, v);
            if (!this.columns || (this.columns && ui.getValue().length === ui.store.count)) {
              ui.$nextTick(() => {
                ui.$emit('on-change', ui.getValue());
              });
            }
            if (ui.columns !== 0) {
              ui.renderChain(i + 1);
            }
          },
        });
        if (ui.currentValue) {
          ui.scroller[i].select(value[i]);
        }
      }
    },
    renderChain(i) {
      if (!this.columns) {
        return;
      }

      // do not render for last scroller
      if (i > this.count - 1) {
        return;
      }

      const ui = this;
      const ID = this.getId(i);
      // destroy old one
      this.scroller[i].destroy();
      const list = this.store.getChildren(ui.getValue()[i - 1]);
      this.scroller[i] = new Scroller(ID, {
        data: list,
        itemClass: ui.item_class,
        onSelect(value) {
          ui.$set(ui.currentValue, i, value);
          ui.$nextTick(() => {
            ui.$emit('on-change', ui.getValue());
          });
          ui.renderChain(i + 1);
        },
      });
      this.$set(this.currentValue, i, list[0].value);
      this.renderChain(i + 1);
    },
    getValue() {
      const data = [];
      for (let i = 0; i < this.currentData.length; i += 1) {
        if (this.scroller[i]) {
          data.push(this.scroller[i].value);
        } else {
          return [];
        }
      }
      return data;
    },
    emitValueChange(val) {
      if (!this.columns || (this.columns && val.length === this.store.count)) {
        this.$emit('on-change', val);
      }
    },
  },
  data() {
    return {
      scroller: [],
      count: 0,
      uuid: '',
      currentData: this.data,
      currentValue: this.value,
    };
  },
  watch: {
    value(val) {
      if (JSON.stringify(val) !== JSON.stringify(this.currentValue)) {
        this.currentValue = val;
      }
    },
    currentValue(val, oldVal) {
      this.$emit('input', val);
      // render all the scroller for chain datas
      if (this.columns !== 0) {
        if (val.length > 0) {
          if (JSON.stringify(val) !== JSON.stringify(oldVal)) {
            this.currentData = this.store.getColumns(val);
            this.$nextTick(() => {
              this.render(this.currentData, val);
            });
          }
        }
      } else if (val.length) {
        for (let i = 0; i < val.length; i += 1) {
          if (this.scroller[i] && this.scroller[i].value !== val[i]) {
            this.scroller[i].select(val[i]);
          }
        }
      } else {
        this.render(this.currentData, []);
      }
    },
    data(val) {
      if (JSON.stringify(val) !== JSON.stringify(this.currentData)) {
        this.currentData = val;
      }
    },
    currentData(newData) {
      if (Object.prototype.toString.call(newData[0]) === '[object Array]') {
        this.$nextTick(() => {
          this.render(newData, this.currentValue);
          // emit on-change after rerender
          this.$nextTick(() => {
            this.emitValueChange(this.getValue());

            if (JSON.stringify(this.getValue()) !== JSON.stringify(this.currentValue)) {
              if (!this.columns || (this.columns && this.getValue().length === this.store.count)) {
                this.currentValue = this.getValue();
              }
            }
          });
        });
      } else if (this.columns !== 0) {
        if (!newData.length) {
          return;
        }
        const length = this.columns;
        this.store = new Manager(newData, length, this.fixedColumns);
        this.currentData = this.store.getColumns(this.currentValue);
      }
    },
  },
  beforeDestroy() {
    for (let i = 0; i < this.count; i += 1) {
      this.scroller[i] && this.scroller[i].destroy();
      this.scroller[i] = null;
    }
  },
};
</script>
